...
func addParamCommands() (res *cobra.Command) {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			if resp, err := session.Params.ListParams(params.NewListParamsParams()); err != nil {
	...
	*/
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if resp, err := session.Params.GetParam(params.NewGetParamParams().WithName(args[0])); err != nil {
	...
	*/
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if resp, err := session.Params.CreateParam(params.NewCreateParamParams().WithBody(param)); err != nil {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if resp, err := session.Params.GetParam(params.NewGetParamParams().WithName(args[0])); err != nil {
				...
				param = &models.Param{}
				err = yaml.Unmarshal(merged, param)
				if err != nil {
					log.Fatalf("Unable to unmarshal merged object: %v\n", err)
				}

				if resp, err := session.Params.PutParam(params.NewPutParamParams().WithName(args[0]).WithBody(param)); err != nil {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if resp, err := session.Params.PatchParam(params.NewPatchParamParams().WithName(obj.Name).WithBody(p)); err != nil {
				log.Fatalf("Unable to patch %v\n%v\n", args[0], err)
			} else {
				fmt.Println(pretty(resp.Payload))
			}
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Params.DeleteParam(params.NewDeleteParamParams().WithName(args[0])); err != nil {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Params.GetParam(params.NewGetParamParams().WithName(args[0])); err != nil {
				log.Fatalf("Failed to fetch %v: %v\n%v\n", singularName, args[0], err)
			} else {
				os.Exit(0)
			}
