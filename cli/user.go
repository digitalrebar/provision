	...
	*/
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Users.DeleteUser(users.NewDeleteUserParams().WithName(args[0])); err != nil {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Users.GetUser(users.NewGetUserParams().WithName(args[0])); err != nil {
				log.Fatalf("Failed to fetch %v: %v\n%v\n", singularName, args[0], err)
			} else {
				os.Exit(0)
			}
