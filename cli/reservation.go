	...
	*/
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Reservations.DeleteReservation(reservations.NewDeleteReservationParams().WithAddress(s)); err != nil {
	...
	commands = append(commands, &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if _, err := session.Reservations.GetReservation(reservations.NewGetReservationParams().WithAddress(s)); err != nil {
				log.Fatalf("Failed to fetch %v: %v\n%v\n", singularName, args[0], err)
			} else {
				os.Exit(0)
			}
